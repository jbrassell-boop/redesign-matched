# OM05-1 Flexible Endoscope Diagnostic Report Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build a print-only React form component for OM05-1 (Flexible Endoscope Diagnostic Report) and wire it into the existing forms menu in RepairDetailPane.

**Architecture:** New standalone component `DiFlexibleDiagnosticForm.tsx` following the identical pattern as `DiInspectionForm.tsx` and `DiRigidForm.tsx` — fixed modal overlay, Print/Close buttons hidden on print, two `print-page` divs for the 2-page layout. No state, no backend. RepairDetailPane gets a new `'di-flex-diagnostic'` key added to the type union, menu lists, and form renders.

**Tech Stack:** React 19, TypeScript, inline styles (no CSS modules), existing `print.css` classes (`print-form flex-col`, `print-page`, `no-print`)
...
|--------|------|-------------|
| Create | `client/src/pages/repairs/forms/DiFlexibleDiagnosticForm.tsx` | New 2-page print form component |
| Modify | `client/src/pages/repairs/RepairDetailPane.tsx` | Import, type union (line 94), two menu entries (lines ~365, ~612), two form renders (lines ~404, ~658) |

---

## Task 1: Build `DiFlexibleDiagnosticForm.tsx`

**Files:**
- Create: `client/src/pages/repairs/forms/DiFlexibleDiagnosticForm.tsx`
...
Expected: no errors. If errors appear, fix unused imports or type mismatches before continuing.

- [ ] **Step 3: Commit**

```bash
git add client/src/pages/repairs/forms/DiFlexibleDiagnosticForm.tsx
git commit -m "feat: add OM05-1 Flexible Endoscope Diagnostic Report form (print-only)"
```

---

## Task 2: Wire into `RepairDetailPane.tsx`

**Files:**
- Modify: `client/src/pages/repairs/RepairDetailPane.tsx`

- [ ] **Step 1: Add import at line ~11 (after existing form imports)**
...
- [ ] **Step 2: Extend the activeForm type union (line ~94)**
...
- [ ] **Step 3: Add menu entry — first render path (line ~364)**
...
- [ ] **Step 4: Add form render — first render path (line ~404)**
...
- [ ] **Step 5: Add menu entry — second render path (line ~611)**
...
- [ ] **Step 6: Add form render — second render path (line ~658)**
...
git commit -m "feat: wire OM05-1 D&I Flex Diagnostic form into repairs forms menu"
```
...
- [ ] **Step 2: Confirm TypeScript build passes in CI**

Watch GitHub Actions deploy pipeline. The `cd client && npx tsc --noEmit` step must pass. If it fails, read the error and fix unused imports or type issues.

- [ ] **Step 3: Smoke test on live frontend**

Load `https://happy-plant-03638db0f.6.azurestaticapps.net`, navigate to Repairs, open any flexible scope repair, click the Forms button, select "D&I Flex Diagnostic (OM05-1)", confirm the 2-page form renders with the repair's WO#, client, model, serial pre-filled.
