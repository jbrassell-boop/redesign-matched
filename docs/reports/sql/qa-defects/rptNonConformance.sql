CREATE PROCEDURE [dbo].[rptNonConformance]
	(
...
BEGIN
	...
	--exec dbo.rptNonConformance '1/1/2024', '12/31/2024', 0
	...
	FROM dbo.tblClient c JOIN dbo.tblDepartment d ON c.lClientKey = d.lClientKey
		...
		Left Join dbo.tblUsers UsersEval on (i.lEvalUserKey=UsersEval.lUserKey)
		Left Join dbo.tblUsers UsersResp on (i.lResponsibleMgrUserKey=UsersResp.lUserKey)
		Left Join dbo.tblUsers UsersQARev on (i.lFnlDispQAUserKey=UsersQARev.lUserKey)
		Left Join dbo.tblUsers UsersRec on (i.lRecvdByUserKey=UsersRec.lUserKey)
		Left Join dbo.tblUsers UsersVOE on (i.lVOEUserKey=UsersVOE.lUserKey)
	...
	if @lDatabaseKey = 2
		BEGIN
			Insert Into #Results EXEC TSI.WinscopeNet.dbo.rptNonConformanceForSouth @pdtStartDate, @pdtEndDate, @plRepairKey
		END

	Select * From #Results Order By dtDateIn DESC, sWorkOrderNumber DESC
	Drop Table #Results
END
