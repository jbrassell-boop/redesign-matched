CREATE PROCEDURE [dbo].[portalRptDataSrcRepairFnlIns_Total]
	(
		@plRepairKey		INT,
		@prmPortal			tinyint
	)
AS
BEGIN
	SELECT SUM(IsNull(dblRepairPrice,0)) As RepairTotal FROM
		(
		SELECT	FI.*, RE.mComments AS mCommentsVisible, SC.mComments, RE.mCommentsHidden
		FROM	(
				SELECT	1 AS nItemType,
						...
						RT.sComments, Case When RT.sApproved='Y' Then 'Y' Else 'N' End AS sApproved, 
						Case 
