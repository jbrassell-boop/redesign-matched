...
import { getDeptFlags, addDeptFlag, updateDeptFlag, deleteDeptFlag } from '../../../api/departments';
...
export const FlagsTab = ({ deptKey }: FlagsTabProps) => {
  ...
  const handleSave = async () => {
    ...
    if (editRow.isNew) {
      await addDeptFlag(deptKey, payload);
    } else {
      await updateDeptFlag(deptKey, editRow.flagKey, payload);
    }
    reload();
  };
  ...
  const handleDelete = (flagKey: number) => {
    Modal.confirm({
      ...
      onOk: async () => {
        await deleteDeptFlag(deptKey, flagKey);
        reload();
      },
    });
  };
