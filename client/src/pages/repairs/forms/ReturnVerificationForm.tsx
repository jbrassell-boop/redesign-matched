        ...
        <div style={{ padding: '0.5in', display: 'flex', flexDirection: 'column', gap: 10 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 8 }}>
            ...
            <div style={{ textAlign: 'right' }}>
              ...
              <div style={{ fontSize: 10, color: 'var(--print-light)', marginTop: 2 }}>OM14-1</div>
            </div>
          </div>
            ...
            <tbody>
              {Array.from({ length: 6 }).map((_, i) => (
                <tr key={i} style={i % 2 === 1 ? { background: 'var(--bg)' } : undefined}>
                  ...
                  <td style={{ ...tdS, textAlign: 'center' }}><span style={{ display: 'inline-block', width: 14, height: 14, border: '1px solid var(--print-check-border)', borderRadius: 2 }} /></td>
          ...
          <div style={{ marginTop: 10, padding: '10px 14px', background: 'var(--print-warn-bg)', border: '1.5px solid var(--warning)', borderRadius: 4 }}>
            <div style={{ fontSize: '9.5px', fontWeight: 800, textTransform: 'uppercase', color: 'var(--badge-amber-text)', letterSpacing: '.06em', marginBottom: 4 }}>Disinfection Reminder</div>
            <div style={{ fontSize: '10.5px', color: 'var(--print-warn-text)', lineHeight: 1.5 }}>
...
const Bar = ({ children, style }: { children: React.ReactNode; style?: React.CSSProperties }) => (
  <div style={{ background: 'var(--primary)', color: 'var(--card)', fontSize: 9, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.06em', padding: '4px 10px', ...style }}>{children}</div>
);
...
const Fld = ({ label, value, span2 }: { label: string; value?: string | null; span2?: boolean }) => (
  <div style={{ display: 'flex', flexDirection: 'column', gap: 1, ...(span2 ? { gridColumn: 'span 2' } : {}) }}>
    ...
    <div style={{ borderBottom: '1px solid var(--print-check-border)', minHeight: 17, fontSize: 11, padding: '1px 2px' }}>{value || ''}</div>
  </div>
);

const AddrBlock = ({ title, name, addr, csz }: { title: string; name: string; addr: string; csz: string }) => (
  <div style={{ border: '1px solid var(--print-border-lt)', borderRadius: 4, padding: '8px 12px' }}>
    <div style={{ fontSize: '8.5px', fontWeight: 700, textTransform: 'uppercase', color: 'var(--primary)', letterSpacing: '.06em', marginBottom: 6, borderBottom: '1px solid var(--print-border-xlt)', paddingBottom: 3 }}>{title}</div>
    <div style={{ borderBottom: '1px solid var(--print-border)', minHeight: 16, fontSize: 11, marginBottom: 4 }}>{name}</div>
    <div style={{ borderBottom: '1px solid var(--print-border)', minHeight: 16, fontSize: 11, marginBottom: 4 }}>{addr}</div>
    <div style={{ borderBottom: '1px solid var(--print-border)', minHeight: 16, fontSize: 11, marginBottom: 4 }}>{csz}</div>
  </div>
);
...
const Sig = ({ label, width }: { label: string; width?: number }) => (
  <div style={{ flex: width ? undefined : 1, maxWidth: width, display: 'flex', flexDirection: 'column', gap: 2 }}>
    <div style={{ borderBottom: '1px solid var(--print-check-border)', minHeight: 30 }} />
