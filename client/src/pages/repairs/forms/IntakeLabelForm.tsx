...
const labelStyle: React.CSSProperties = {
  ...
  border: '1px dashed var(--print-border)', margin: '0 auto', overflow: 'hidden',
  fontFamily: 'Arial, sans-serif', background: 'var(--card)', color: 'var(--print-text)',
};
...
export const IntakeLabelForm = ({ repair, onClose }: Props) => (
  ...
  >
    ...
    <div className="print-form" style={labelStyle}>
      ...
      <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', borderBottom: '1.5px solid var(--print-text)', paddingBottom: 4, marginBottom: 2 }}>
      ...
      {([['Client', repair.client], ['Model', repair.scopeModel], ['Serial', repair.serial]] as [string, string | undefined | null][]).map(([lbl, val]) => (
        <div key={lbl} style={{ display: 'flex', alignItems: 'baseline', gap: 6 }}>
          <span style={{ fontSize: 7.5, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--print-muted)', width: 44, flexShrink: 0 }}>{lbl}</span>
      ...
      <div style={{ marginTop: 'auto', textAlign: 'center' }}>
        ...
        <div style={{ fontSize: 7.5, color: 'var(--print-muted)', letterSpacing: '0.08em', marginTop: 1 }}>{repair.wo ? `${repair.wo} · OM04-1` : 'OM04-1'}</div>
      </div>
    </div>
  </div>
);
