...
export const UpdateSlipForm = ({ repair, onClose }: Props) => {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 1000, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', overflow: 'auto', padding: '20px 0' }}>
      <div className="print-form" style={{ width: '8.5in', background: 'var(--card)', fontFamily: "'Inter', Arial, sans-serif", fontSize: 11, color: 'var(--print-text)' }}>
        ...
        <div style={{ padding: '0.5in', display: 'flex', flexDirection: 'column', gap: 10 }}>
          {/* Internal Use Banner */}
          <div style={{ background: 'var(--danger-light)', border: '2px solid var(--badge-red-border)', borderRadius: 4, padding: '8px 16px', textAlign: 'center' }}>
            <div style={{ fontSize: 13, fontWeight: 800, color: 'var(--danger)', textTransform: 'uppercase', letterSpacing: '.12em' }}>Internal Use Only</div>
            <div style={{ fontSize: 9, color: 'var(--print-error-text)', marginTop: 2, fontWeight: 600 }}>Do not send to customer — for TSI technician use only</div>
          </div>
