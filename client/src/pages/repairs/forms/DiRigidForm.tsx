        ...
        <div style={{ padding: '0.5in', display: 'flex', flexDirection: 'column', gap: 7 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 6 }}>
            ...
            <div style={{ textAlign: 'right' }}>
              ...
              <div style={{ fontSize: 11, fontWeight: 600, color: '#2E75B6', marginTop: 1 }}>Rigid Endoscope</div>
...
const PfBtn = ({ pass, fail: _fail }: { pass?: boolean; fail?: boolean }) => (
  <span style={{
    display: 'inline-block', width: 22, height: 14, border: `1px solid ${pass ? '#16A34A' : '#B71234'}`,
    borderRadius: 2, textAlign: 'center', lineHeight: '14px', fontSize: 8, fontWeight: 700,
    color: pass ? '#16A34A' : '#B71234',
  }}>{pass ? 'P' : 'F'}</span>
);
...
const thS: React.CSSProperties = { background: '#2E75B6', color: '#fff', fontSize: 8, fontWeight: 700, textTransform: 'uppercase', padding: '3px 5px', textAlign: 'center', letterSpacing: '.03em', borderRight: '1px solid rgba(255,255,255,.2)' };
...
const RigidTable = () => (
  <table style={{ width: '100%', borderCollapse: 'collapse', marginTop: 4, fontSize: '9.5px' }}>
    ...
    <tbody>
      {GROUPS.map(g => (
        <>
          <tr key={g.cat} style={{ background: '#E8F0FE' }}>
            <td colSpan={11} style={{ fontSize: 8, fontWeight: 700, textTransform: 'uppercase', color: '#1B3A5C', letterSpacing: '.04em', padding: '3px 5px' }}>{g.cat}</td>
          </tr>
          {g.items.map((item, i) => (
