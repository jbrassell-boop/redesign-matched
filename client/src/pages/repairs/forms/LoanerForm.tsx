...
export const LoanerForm = ({ repair, onClose }: Props) => {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 1000, background: 'rgba(0,0,0,0.4)', backdropFilter: 'blur(2px)', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', overflow: 'auto', padding: '20px 0' }}>
      <div className="print-form" style={{ width: '8.5in', background: 'var(--card)', fontFamily: "'Inter', Arial, sans-serif", fontSize: '10.5px', color: 'var(--print-text)' }}>
        {/* Print/Close */}
        ...
        <div style={{ padding: '0.45in', display: 'flex', flexDirection: 'column', gap: 8 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 6 }}>
            ...
            <div style={{ textAlign: 'right' }}>
              ...
              <div style={{ fontSize: 10, color: 'var(--print-light)', marginTop: 2 }}>OM17-1</div>
            </div>
          </div>
            ...
            <tbody>
              {Array.from({ length: 3 }).map((_, i) => (
                <tr key={i} style={i % 2 === 1 ? { background: 'var(--bg)' } : undefined}>
                  <td style={{ padding: '4px 7px', fontSize: 10, borderBottom: '1px solid var(--print-border-lt)', minHeight: 18 }}>&nbsp;</td>
                  <td style={{ padding: '4px 7px', fontSize: 10, borderBottom: '1px solid var(--print-border-lt)' }}>&nbsp;</td>
                  <td style={{ padding: '4px 7px', fontSize: 10, borderBottom: '1px solid var(--print-border-lt)' }}>&nbsp;</td>
                  <td style={{ padding: '4px 7px', fontSize: 10, borderBottom: '1px solid var(--print-border-lt)' }}>&nbsp;</td>
                  <td style={{ padding: '4px 7px', fontSize: 10, borderBottom: '1px solid var(--print-border-lt)' }}>&nbsp;</td>
                </tr>
          ...
          <div>
            <div style={{ background: 'var(--print-warn-bg)', border: '1.5px solid var(--warning)', borderRadius: 3, padding: '5px 10px', fontSize: '9.5px', fontWeight: 700, color: 'var(--badge-amber-text)', margin: '3px 0' }}>
              Return deadline: 5 business days from delivery. Late fee: $200.00 per day after due date.
            </div>
            <div style={{ fontSize: '9.5px', color: 'var(--print-body)', lineHeight: 1.5, padding: '6px 10px', background: 'var(--bg)', border: '1px solid var(--print-border-xlt)', borderRadius: 3 }}>
          ...
          <div style={{ marginTop: 6, fontSize: '9.5px', color: 'var(--print-subtle)', padding: '5px 10px', background: 'var(--bg)', border: '1px solid var(--print-border-xlt)', borderRadius: 3 }}>
          ...
          {/* Footer */}
          <div style={{ marginTop: 'auto', paddingTop: 6, borderTop: '1px solid var(--print-border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: 8, color: 'var(--print-footer)' }}>
...
const Fld = ({ label, value, span2 }: { label: string; value?: string | null; span2?: boolean }) => (
  <div style={{ display: 'flex', flexDirection: 'column', gap: 1, ...(span2 ? { gridColumn: 'span 2' } : {}) }}>
    ...
    <div style={{ borderBottom: '1px solid var(--print-check-border)', minHeight: 16, fontSize: '10.5px', padding: '1px 2px' }}>{value || ''}</div>
  </div>
);
...
const Sig = ({ label, width }: { label: string; width?: number }) => (
  <div style={{ flex: width ? undefined : 1, maxWidth: width, display: 'flex', flexDirection: 'column', gap: 2 }}>
    ...
    <div style={{ fontSize: 8, color: 'var(--print-muted)', fontWeight: 600, marginTop: 2 }}>{label}</div>
  </div>
);
